import React from "react";
import "bootstrap/dist/css/bootstrap.min.css";
import dw2 from "../../images/V9.jpg";
import dw3 from "../../images/v2.jpg";
import dw4 from "../../images/v4.jpg";
import dw5 from "../../images/v3.JPG";
import dw6 from "../../images/v8.jpg";
import dw1 from "../../images/v6.JPG";

const VideographyGallery = () => {
  return (
    <div className="wrapper">

      <h2 className="heading">
        <span className="circle title">
          Videography &nbsp; <span className="subTitle"> Gallery</span>
        </span>
      </h2>
      <div className="row g-4 mt-3">
        {/* First Column - 1 Large Image (Spans 2 rows) */}
        <div className="col-md-4 d-flex flex-column">
          <img
            src={dw1}
            className="img-fluid w-100 flex-fill rounded"
            alt="Dental Image 1"
            style={{ objectFit: "cover", height: "300px", marginBottom: "16px" }}
          />
          <img
            src={dw4}
            className="img-fluid w-100 flex-fill rounded"
            alt="Dental Image 2"
            style={{ objectFit: "cover", height: "300px" }}
          />
        </div>

        {/* Second Column - 2 Images Stacked */}
        <div className="col-md-4 d-flex flex-column">
          <img
            src={dw3}
            className="img-fluid w-100 flex-fill rounded"
            alt="Dental Image 2"
            style={{ objectFit: "cover", height: "300px", marginBottom: "16px" }}
          />
          <img
            src={dw2}
            className="img-fluid w-100 flex-fill rounded"
            alt="Dental Image 3"
            style={{ objectFit: "cover", height: "300px" }}
          />
        </div>

        {/* Third Column - 2 Images Stacked */}
        <div className="col-md-4 d-flex flex-column">
          <img
            src={dw5}
            className="img-fluid w-100 flex-fill rounded"
            alt="Dental Image 4"
            style={{ objectFit: "cover", height: "300px", marginBottom: "16px" }}
          />
          <img
            src={dw6}
            className="img-fluid w-100 flex-fill rounded"
            alt="Dental Image 5"
            style={{ objectFit: "cover", height: "300px" }}
          />
        </div>
      </div>
    </div>
  );
};

export default VideographyGallery;
